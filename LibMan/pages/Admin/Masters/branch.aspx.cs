...
namespace LibMan.pages.Admin.Masters
{
    public partial class branch : System.Web.UI.Page
    {
        ...
        protected void btn_save_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (txt_branch.Value != "")
                lst.Add(new ViewParam() { Name = "@branch", Value = txt_branch.Value });

            lst.Add(new ViewParam() { Name = "@usrID", Value = Common.Decrypt(Request.Cookies["LibID"].Value, true) });
            DS = DM.View.GetDataSet("[L_Branch_save]", lst);
        ...
        public static string GetBranchByID(string id)
        {
            ...
            DS = DM.View.GetDataSet("L_Branch_get", lst);
            return Common.ConverDatasetToJson(DS);
        }
        [WebMethod]
        public static string DeleteUpdateBranch(string id, string flag, bool status)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
        ...
        [WebMethod]
        public static string getAllBranchs()
        {
            List<ViewParam> _lst = new List<ViewParam>();
            DataSet ds = new DataSet();
            DataManager dm = new DataManager();
            ds = dm.View.GetDataSet("L_Branch_get", _lst);
            return Common.ConverDatasetToJson(ds);
        }
