...
namespace LibMan.pages.Admin.Masters
{
    public partial class itemgroup : System.Web.UI.Page
    {
        ...
        protected void btn_save_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (txt_itemGroup.Value != "")
                lst.Add(new ViewParam() { Name = "@itemGroup", Value = txt_itemGroup.Value });

            lst.Add(new ViewParam() { Name = "@usrID", Value = Common.Decrypt(Request.Cookies["LibID"].Value, true) });
            DS = DM.View.GetDataSet("[L_ItemGroup_save]", lst);
        ...
        public static string GetItemGroupByID(string id)
        {
            ...
            DS = DM.View.GetDataSet("L_ItemGroup_get", lst);
            return Common.ConverDatasetToJson(DS);
        }
        [WebMethod]
        public static string DeleteUpdateItemGroup(string id, string flag, bool status)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
        ...
        [WebMethod]
        public static string getAllItemGroups()
        {
            List<ViewParam> _lst = new List<ViewParam>();
            DataSet ds = new DataSet();
            DataManager dm = new DataManager();
            ds = dm.View.GetDataSet("L_ItemGroup_get", _lst);
            return Common.ConverDatasetToJson(ds);
        }
