...
namespace LibMan.pages.Admin.Masters
{
    public partial class rack : System.Web.UI.Page
    {
        ...
        protected void btn_save_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (txt_rack.Value != "")
                lst.Add(new ViewParam() { Name = "@rack", Value = txt_rack.Value });

            lst.Add(new ViewParam() { Name = "@usrID", Value = Common.Decrypt(Request.Cookies["LibID"].Value, true) });
            DS = DM.View.GetDataSet("[L_Rack_save]", lst);
        ...
        [WebMethod]
        public static string GetRackByID(string id)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
            lst.Add(new ViewParam() { Name = "@id", Value = id });
            DS = DM.View.GetDataSet("L_Rack_get", lst);
            return Common.ConverDatasetToJson(DS);
        }
        [WebMethod]
        public static string DeleteUpdateRack(string id, string flag, bool status)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
        ...
        [WebMethod]
        public static string getAllRacks()
        {
            List<ViewParam> _lst = new List<ViewParam>();
            DataSet ds = new DataSet();
            DataManager dm = new DataManager();
            ds = dm.View.GetDataSet("L_Rack_get", _lst);
            return Common.ConverDatasetToJson(ds);
        }
