    ...
    public partial class department : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                DS = DM.View.GetDataSet("[L_Department_get]", lst);
                if (DS != null)
                {
                    if (DS.Tables.Count > 0)
                    {
                        ddl_incharge.DataSource = DS.Tables[0];
                        ddl_incharge.DataValueField = "staff_kid";
                        ddl_incharge.DataTextField = "staff_name";
                        ddl_incharge.DataBind();
                    }
                    ddl_incharge.Items.Insert(0, new ListItem("---Select---", "0"));
        ...
        protected void btn_save_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (ddl_incharge.SelectedValue != "")
                lst.Add(new ViewParam() { Name = "@incharge", Value = ddl_incharge.SelectedValue });

            if (txt_department.Value != "")
                lst.Add(new ViewParam() { Name = "@department", Value = txt_department.Value });

            lst.Add(new ViewParam() { Name = "@usrID", Value = Common.Decrypt(Request.Cookies["LibID"].Value, true) });
            DS = DM.View.GetDataSet("[L_Department_save]", lst);
        ...
        [WebMethod]
        public static string DeleteUpdateDepartment(string id, string flag, bool status)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
        ...
        public static string getAllDepartments()
        {
            ...
            DataSet ds = new DataSet();
            DataManager dm = new DataManager();
            ds = dm.View.GetDataSet("L_Department_get", _lst);
            return Common.ConverDatasetToJson(ds);
        }
