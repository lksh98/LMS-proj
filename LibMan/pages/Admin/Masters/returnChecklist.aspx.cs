...
{
    public partial class returnChecklist : System.Web.UI.Page
    {
        ...
        protected void btn_save_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (txt_returnChecklist.Value != "")
                lst.Add(new ViewParam() { Name = "@returnChecklist", Value = txt_returnChecklist.Value });

            lst.Add(new ViewParam() { Name = "@usrID", Value = Common.Decrypt(Request.Cookies["LibID"].Value, true) });
            DS = DM.View.GetDataSet("[L_ReturnChecklist_save]", lst);
        ...
        [WebMethod]
        public static string GetReturnChecklistByID(string id)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
            lst.Add(new ViewParam() { Name = "@id", Value = id });
            DS = DM.View.GetDataSet("L_ReturnChecklist_get", lst);
            return Common.ConverDatasetToJson(DS);
        }
        [WebMethod]
        public static string DeleteUpdateReturnChecklist(string id, string flag, bool status)
        {
            List<ViewParam> lst = new List<ViewParam>();
            DataSet DS = new DataSet();
            DataManager DM = new DataManager();
        ...
        [WebMethod]
        public static string getAllReturnChecklists()
        {
            List<ViewParam> _lst = new List<ViewParam>();
            DataSet ds = new DataSet();
            DataManager dm = new DataManager();
            ds = dm.View.GetDataSet("L_ReturnChecklist_get", _lst);
            return Common.ConverDatasetToJson(ds);
        }
