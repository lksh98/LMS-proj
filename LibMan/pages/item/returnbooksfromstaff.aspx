<%@ Page Title="Return Books from Staff - Lib-Man" Language="C#" MasterPageFile="/pages/Admin/Dashboard/Admin.Master" AutoEventWireup="true" CodeBehind="returnbooksfromstaff.aspx.cs" EnableEventValidation="false" Inherits="LibMan.pages.item.returnbooksfromstaff" %>
    ...
    <section class="mb-2">
        <div class="row  border-bottom-1 pb-0">
            ...
            <div class="col-md-9 col-sm-8 col-8 ">
                <h3 id="Book_head" class="heading-modal">Return Books from Staff </h3>
            </div>

        </div>
        <div class="row mt-4">
            <div class="col-md-1 mt-1 col-sm-3">
                <label class="input-label-modal">Staff ID </label>
            </div>
            <div class="col-md-2 mt-1 col-sm-9">
                <input type="text" maxlength="50" id="txt_staffID" runat="server" tabindex="1" placeholder="..............................................." class="txt_staffID BlankText   isbn all-border border-bottom-2 font-12 width-100">
            </div>
            <div class="col-md-1 mt-1 col-sm-3">
                <label class="input-label-modal">Staff Name </label>
            </div>
            <div class="col-md-2 mt-1 col-sm-9">
                <input type="text" maxlength="50" id="txt_staffname" readonly="readonly" runat="server" tabindex="-1" placeholder="..............................................." class="txt_staffname       all-border border-bottom-2 font-12 width-100">
            </div>
            <div class="col-md-1 mt-1 col-sm-3">
...
<asp:Content runat="server" ContentPlaceHolderID="ContentPlaceHolder_BottomScripts">
    <script>
        ...
        <%--$(document).on('change', '#<%=txt_staffID.ClientID%>', DataBind);--%>

        function DataBind(id) {
            $.ajax({
                type: "POST",
                url: 'returnbooksfromstaff.aspx/getIssuedBooks',
                data: "{'StaffID':'" + id + "'}",
        ...
        function OnSuccess(response) {
            ...
            table = $('#tbl_Book').DataTable({
                ...
                { data: 'book_publisher', sortable: true, searchable: true },
                    { data: 'issueTostaff_issuedate', sortable: true, searchable: true },
                    { data: 'issueTostaff_rdate', sortable: true, searchable: true },
                { data: 'usr_name', sortable: true, searchable: true },
                { data: 'fine', sortable: true, searchable: true }
                ],
                "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                    if (aData.issueTostaff_status == "I") {
                        if (aData.Late == 'L') {
            ...
            if (Fn_Save()) {
                if ($('#<%=txt_fine.ClientID%>').val() != "" && $('#<%=txt_fine.ClientID%>').val() != "0") {
                    swal({
                        ...
                        function (isConfirm) {
                            if (isConfirm) {
                                var EditBook = jqPost("returnbooksfromstaff.aspx/saveBook", "{ 'issueID':'" + $('#<%=hdn_issueID.ClientID%>').val() + "','remark':'" + $('#<%=txt_remark.ClientID%>').val() + "','fine':'" + $('#<%=txt_fine.ClientID%>').val() + "'}");
        ...
        $(document).on('change', '#<%=txt_bookID.ClientID%>', function () {
            var bookid = $(this).val();
            var txt_staffID = $('#<%=txt_staffID.ClientID%>').val();
            var EditBook = jqPost("returnbooksfromstaff.aspx/getBookDetails", "{'bookid':'" + bookid + "', 'StaffID':'" + txt_staffID + "'}");
            ...
            if (EditBook != null && EditBook.Table1.length > 0) {
                ...
                $('#<%=txt_rdate.ClientID%>').val(EditBook.Table1[0].issueTostaff_rdate);
                $('#<%=txt_fine.ClientID%>').val(EditBook.Table1[0].fine);
                $('#<%=hdn_issueID.ClientID%>').val(EditBook.Table1[0].issueTostaff_kid);
        ...
        $(document).on('change', '#<%=txt_staffID.ClientID%>', function () {
            var txt_staffID = $(this).val();

            var EditBook = jqPost("returnbooksfromstaff.aspx/getStaffDetails", "{'StaffID':'" + txt_staffID + "'}");
            console.log(EditBook)
            if (EditBook != null && EditBook.Table1.length > 0) {
                $('#<%=hdn_id.ClientID%>').val(EditBook.Table1[0].staff_kid);
                $('#<%=txt_staffname.ClientID%>').val(EditBook.Table1[0].staff_name);
                DataBind(EditBook.Table1[0].staff_kid)
            }
            else {
                MsgArray[0] = "Staff ID does not exists.";
                MsgColor(true);
                MsgAlert();
                $('#<%=hdn_id.ClientID%>').val('');
                $('#<%=txt_staffID.ClientID%>').val("");
                $('#<%=txt_staffname.ClientID%>').val("");
                DataBind(0)
            }
