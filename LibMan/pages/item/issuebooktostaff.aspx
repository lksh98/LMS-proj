<%@ Page Title="Issue Book to Staff- Lib-Man" Language="C#" MasterPageFile="/pages/Admin/Dashboard/Admin.Master" AutoEventWireup="true" CodeBehind="issuebooktostaff.aspx.cs" EnableEventValidation="false" Inherits="LibMan.pages.item.issuebooktostaff" %>
    ...
    <section class="mb-2">
        <div class="row  border-bottom-1 pb-0">
            ...
            <div class="col-md-9 col-sm-8 col-8 ">
                <h3 id="Book_head" class="heading-modal">Issue Book to Staff </h3>
            </div>
...
<asp:Content runat="server" ContentPlaceHolderID="ContentPlaceHolder_BottomScripts">
    <script>
        ...
        function DataBind(id) {
            $.ajax({
                type: "POST",
                url: 'issuebooktostaff.aspx/getIssuedBooks',
                data: "{'StaffID':'" + id + "'}",
        ...
        function OnSuccess(response) {
            ...
            $('#<%=hdn_current.ClientID%>').val(data.Table1.filter(e => {return e.issueToStaff_status == "I" }).length);
            table = $('#tbl_Book').DataTable({
                ...
                "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                    ...
                    else if (aData.issueToStaff_status == "R") {
        ...
        $(document).on('click', '.addRecord', function () {
            ...
            if (Fn_Save()) {
                if (max_book == "" || (parseInt(max_book) > parseInt(currentBook))) {
                    var EditBook = jqPost("issuebooktostaff.aspx/saveBook", "{'bookid':'" + $('#<%=hdn_bookStock.ClientID%>').val() + "','StaffID':'" + $('#<%=hdn_id.ClientID%>').val() + "'}");
        ...
        $(document).on('change', '#<%=txt_bookID.ClientID%>', function () {
            ...
            var EditBook = jqPost("issuebooktostaff.aspx/getBookDetails", "{'bookid':'" + bookid + "'}");
            console.log(EditBook)
            if (EditBook != null && EditBook.Table1.length > 0) {
        ...
        $(document).on('change', '#<%=txt_staffID.ClientID%>', function () {
            ...
            var EditBook = jqPost("issuebooktostaff.aspx/getStaffDetails", "{'StaffID':'" + txt_staffID + "'}");
            console.log(EditBook)
            if (EditBook != null && EditBook.Table1.length > 0) {
